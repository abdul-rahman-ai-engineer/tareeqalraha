...
const Checkout = () => {
  ...
  useEffect(() => {
    setTotalAmount(
      ...
      ?.toLocaleString('ar-AE', { style: 'currency', currency: 'AED' }
      );
        ...
        .Buttons(
          {
            createOrder: async () => {
              ...
              const resp = await fetch(
                "https://v6.exchangerate-api.com/v6/8f85eea95dae9336b9ea3ce9/latest/AED"
              );
              ...
              if (respData.result === "success") {
                const usdToAedRate = respData.conversion_rates.USD;
                convertedAmount = (totalAmount * usdToAedRate).toFixed(2);
              }
                  ...
                  context?.cartData?.length !== 0 && context?.cartData?.map((item, index) => {
                    return (
                      <div className="flex items-center justify-between py-2" key={index}>
                        ...
                        <span className="text-[14px] font-[500]">{(item?.quantity * item?.price)?.toLocaleString('ar-AE', { style: 'currency', currency: 'AED' })}</span>
                      </div>
                    )
                  })
                }
