...
const AddProduct = () => {
    ...
    const handleChangeProductCat = (event) => {
        ...
        const cat = context?.catData?.find((c) => c._id === value);
        setFormFields((prev) => ({
            ...prev,
            catId: value,
            category: value,
    ...
    const handleChangeProductSubCat = (event) => {
        ...
        setProductThirdLavelCat('');
        let parent, sub;
        for (const c of context?.catData || []) {
            sub = c?.children?.find((ch) => ch._id === value);
            if (sub) {
                parent = c;
                break;
            }
        }
        setProductCat(parent?._id || '');
        setFormFields((prev) => ({
            ...prev,
            catId: parent?._id || '',
            catName: parent?.name || '',
    ...
    const handleChangeProductThirdLavelCat = (event) => {
        ...
        for (const c of context?.catData || []) {
            for (const s of c?.children || []) {
                third = s?.children?.find((t) => t._id === value);
                if (third) {
                    sub = s;
                    cat = c;
                    break;
        ...
        setProductCat(cat?._id || '');
        setProductSubCat(sub?._id || '');
        setFormFields((prev) => ({
            ...prev,
            catId: cat?._id || '',
            catName: cat?.name || '',
